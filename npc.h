...
void InitAllNPC()
{
    ...
    npcs[NPC_LUCY].animFPS = 60;
    npcs[NPC_LUCY].animFrame = 0;
    ...
    npcs[NPC_ABBY].animFPS = 60;
    npcs[NPC_ABBY].animFrame = 0;
    ...
    npcs[NPC_GALADRIEL].animFrame = 0;
    //setup roger
...
#define JARED_MAGIC_RISE_SPEED    3.0f
#define JARED_MAGIC_FALL_SPEED     7.0f
#define JARED_MAGIC_SPIN_DECEL   256
#define JARED_MAGIC_SPIN_ACCEL   1024
#define JARED_MAGIC_MAX_SPIN    2600.0f
...
static inline void NPC_Update_Wiz(NPC* n, const Donogan* d, float dt)
{
    if (n->state == WIZARD_STATE_FLY)
    {
        ...
        if (Vector3DistanceSqr(n->pos, d->pos) > 600*600)
        {
...
static inline void NPC_Update_Rescue(NPC* n, const Donogan* d, float dt, bool looped)
{
    if (n->r_state == RESCUE_STATE_RUN)
    {
        ...
        n->yaw = TurnToward(n->yaw, targetYaw, dt * 6.0f); // gentle turn rate
        //smooth speed move
        Vector3 d = { n->targetPos.x - n->pos.x, 0, n->targetPos.z - n->pos.z };
