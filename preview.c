...
static inline Vector3 Don_GetRecentMoveXZ(const Donogan* d, Vector3 currentPos, float minLen)
{
    ...
    // Second: if current got pinned/reused weirdly, scan history pair-to-pair.
    for (int hb = 1; hb < d->posHistoryCount && hb < DON_POS_HISTORY_MAX; hb++)
    {
...
/// @return 
int main(void) {
    bool dream_land = false;
    Vector3 dream_location = (Vector3){ -9999, 10001, -9999 };
    Vector3 reality_cache = (Vector3) { 0 };
    ...
    while (!WindowShouldClose() && keepAlive)
    {
        ...
        don.level = 1 + (((int)(110.0 * (1.0 - exp(-don.xp / 12000.0)))) / 3); //yep, not confusing at all
        ...
        if (onLoad && donnyMode) //donny collision (probably not all of it, ugh!)
        {
            ...
            for (int i = 0; i < SCENE_TOTAL_COUNT; i++)
            {
                ...
                if (Vector3LengthSqr(contactMove) < 0.02f * 0.02f)
                {
                    contactMove = Don_GetRecentMoveXZ(&don, don.pos, 0.02f);
                }
